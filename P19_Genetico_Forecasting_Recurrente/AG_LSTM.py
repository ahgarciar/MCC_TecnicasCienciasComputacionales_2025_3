...
def create_model(parameters, shape):
    kernel_initializer = glorot_uniform(seed=5)  # RandomUniform(minval=-0.05, maxval=0.05, seed=5)
    recurrent_initializer = Orthogonal(gain=1.0, seed=5)  # RandomUniform(minval=-0.05, maxval=0.05, seed=5)
    bias_initializer = 'zeros'  # Los sesgos se inicializan a cero
...
def exec_lstm(solutions_set, datasets, std):
...
if __name__ == '__main__':
    ...
    results = exec_lstm(solutions_set=solutions_parameters, datasets=data, std=estandarizador)
    ...
    while it < tot_generations:
        ...
        results = exec_lstm(solutions_set=hijos, datasets=data, std=estandarizador)
