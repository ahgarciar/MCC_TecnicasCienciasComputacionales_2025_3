...
multiplicador_positivo = 2.0  # costo de almacenaje
multiplicador_negativo = 6.0  # costo de adquisicion
...
if __name__ == '__main__':
    ...
    model = load_model('Mejor_Modelo/modelo_product_1.keras', custom_objects={
    ...
    inventario_semanal = []
    for valor in diff:
        valor = inventario + np.round(valor[0],0)
        if valor > 0:
            perdida = valor * multiplicador_positivo
            losses.append(perdida)
            loss += perdida
            inventario = valor
            inventario_semanal.append(inventario)
            lossAlmacenaje += perdida
        elif valor < 0:
            valor = valor * -1  # para calcular los productos que se deben adquirir
            inventario = 0
            inventario_semanal.append(0)
            perdida = valor * multiplicador_negativo
            losses.append(perdida)
            loss += perdida
            lossAdquisicion += perdida
        else:
            losses.append(0)
            inventario_semanal.append(0)
    ...
    for i in range(len(y_pred)):
        print(np.round(y_pred[i][0], 0), " ", y_true[i][0], " ", losses[i], " ",
              inventario_semanal[i])

    print("Inventario_Semanal: ", inventario_semanal)
    print("Loss_: ", loss)
    print("Loss_Almacenaje: ", lossAlmacenaje)
    print("Loss_Adquisicion: ", lossAdquisicion)
