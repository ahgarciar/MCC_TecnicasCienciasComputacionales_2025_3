...
def load_instance(instance_name=''):
    global normalization_layer, scalers
...
if __name__ == '__main__':
   ...
   # mape_manual2 = error.calcMAPE(y_true, y_pred)
    ...
    plt.plot(y_pred, linewidth=1.5, label="Forecast")
    plt.plot(y_true, '--', linewidth=1.5, label="Actual")

    ax = plt.gca()
    ax.xaxis.set_major_locator(mticker.MaxNLocator(nbins=15, integer=True))  #

    plt.xlabel("Houe", weight='bold', size=14)
    plt.ylabel("T2M", weight='bold', size=14)
    plt.title("Temperature", weight='bold', size=16)
    plt.legend()
